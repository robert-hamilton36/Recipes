...
describe('POST /add', () => {
  describe('valid request data', () => {
    ...
    test('Functions are called correctly', async () => {
      await request(mockedServer).post('/add').send({userId: 1, recipe: CaramelSliceJSONRecipe})
  ...
  describe('invalid request data', () => {
    describe('no bodyy.userId', () => {
      ...
      test('Functions are not called', async () => {
        await request(mockedServer).post('/add').send({recipe: CaramelSliceJSONRecipe})
        expect(MockedAddRecipe).toHaveBeenCalledTimes(0)
        expect(MockedUserSavesRecipe).toHaveBeenCalledTimes(0)
      })

      test('returns status code 200', async () => {
        const response = await request(mockedServer).post('/add').send({recipe: CaramelSliceJSONRecipe})
    ...
    describe('no body.recipe', () => {
      ...
      test('returns status code 200', async () => {
        const response = await request(mockedServer).post('/add').send({userId: 1})
        expect(response.statusCode).toBe(400)
      })
...
describe('PATCH /edit', () => {
  describe('valid request data', () => {
    ...
    test('Functions are called correctly', async () => {
      await request(mockedServer).patch('/edit/1').send({ editedRecipe: { cookingTime: { quantity: 40 } } })
      expect(MockedUpdateRecipe).toHaveBeenCalledTimes(1)
      expect(MockedUpdateRecipe.mock.calls[0][0]).toBe(1)
      expect(MockedUpdateRecipe.mock.calls[0][1]).toEqual({ cookingTime: { quantity: 40 } })
    })

    test('returns status code 200', async () => {
      const response = await request(mockedServer).patch('/edit/1').send({ editedRecipe: { cookingTime: { quantity: 40 } } })
      expect(response.statusCode).toBe(200)
    })
  ...
  describe('invalid request data', () => {
    ...
    test('returns status code 400 with error message Wrong id', async () => {
      ...
      const response = await request(mockedServer).patch('/edit/1').send({ editedRecipe: { cooking_time: { quantity: null } } })
...
describe('DELETE /unsave', () => {
  describe('valid request data', () => {
    ...
    test('Functions are called correctly', async () => {
      ...
      expect(MockedDeleteItemBySelector.mock.calls[0][1]).toEqual({ user_id: 1, recipe_id: 1 })
    })
