...
import {
  ...
  getFirstItemBySelector,
  getIdByUniqueProperty,
  getItemsBySelector,
...
} from "../../functions/createDatabaseObjects"
...
const __addIngredientToDatabase = async (
  ingredient: JSONIngredient,
  recipeId: number,
  trx: Knex.Transaction
) => {
  ...
  const recipeIngredient = createRecipeIngredientDatabaseObject(
    ingredient,
    recipeId,
    ingredientId
  )
...
export async function getRecipeById(id: number) {
  ...
  const recipeIngredients = await getItemsBySelector("recipe_ingredients", {
    recipe_id: id,
  })
...
export async function updateRecipe(
  recipe_id: number,
  incomingRecipe: Partial<JSONRecipe>
) {
      ...
      if (ingredients) {
        for (const ingredient of ingredients) {
          const editedIngredients =
            createRecipeIngredientDatabaseObject(ingredient)
          if (editedIngredients.ingredient_id === undefined)
            throw new Error("No Id")
          const ingredient_id = editedIngredients.ingredient_id
          await updateItemBySelector(
            "recipe_ingredients",
