...
const createIngredientObject = (
  recipeIngredient: RecipeIngredientDatabase
): JSONIngredient => {
  return {
...
export const createRecipeJSONObject = (
  recipeDatabaseObject: RecipeDatabase,
  recipeIngredients: RecipeIngredientDatabase[]
): JSONRecipe => {
  const ingredients = recipeIngredients.map((recipeIngredient) =>
    createIngredientObject(recipeIngredient)
  )
  return {
